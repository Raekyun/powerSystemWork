...
if __name__ == "__main__":
    df = pd.read_csv('smp_jeju_2020_2021.csv', index_col=0, parse_dates=True, encoding = "ISO-8859-1")
    energyTable = preprocessEpsisData(df)

    pcsCap = 500
    battCap = 2000

    energyTable['smp_for_calc'] = [energyTable['smp'][i] if (17 <= i.hour or i.hour == 0) else 0 for i in
                                   energyTable.index]
