# pvWatts API로 변경
#   pvWatts
#   energy charge, demand charge, fixed charge로 요금 단순화
...
def dailyEssOptPeak(dailyNetLoadPv, pcsCap, battCap, n_t):
    ...
    a[n_t, 0] = 0
    a_ub = a
    b_ub = np.append(-dailyNetLoadPv, battCap)
    x_bounds = [(None, None)]
    for k in range(n_t):
        x_bounds.append((0, pcsCap))
    res = linprog(c, a_ub, b_ub, bounds = x_bounds)
    dailyEssD = np.delete(res.x, 0)
    c = np.zeros(n_t + 1)
    c[0] = 1
    a = np.zeros([n_t + 1, n_t + 1])
    a[:, 0] = -1
    ...
    x_bounds = [(None, None)]
    for k in range(n_t):
        x_bounds.append((-pcsCap, 0))
    res = linprog(c, a_ub, b_ub, bounds = x_bounds)
    dailyEssC = np.delete(res.x, 0)
    dailyEss = dailyEssD + dailyEssC
    return dailyEss
